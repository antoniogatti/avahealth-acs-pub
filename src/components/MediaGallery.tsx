...
export default (props: MediaGalleryProps): JSX.Element => {
  ...
  const calculateNumberOfColumns = React.useCallback(
    (participants) => (participants && participants.length > 0 ? Math.ceil(Math.sqrt(participants.length)) : 1),
    []
  );
  ...
  return (
    ...
    >
          ...
          <div style={{ position: 'absolute', bottom: '2%', left: '2%', width: '300px' }}
              id="localvideostream">
              <div key="localParticipantTile" className={mediaGalleryStyle}>
                  <LocalStreamMedia label={props.displayName} stream={props.localVideoStream} />
              </div>
          </div>
      </div>
